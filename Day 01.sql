...
-- provide Ranking to Price for each country i.e, Window fn. DENSE_RANK()  
-- Group by prod_id
-- order as per result set.

*/

--- My Approach : Using CTE :
...
FROM 
	...
	FROM products as p
	JOIN suppliers as s
	ON s.supplier_id = p.supplier_id) V1 -- using alias for the query 
WHERE rn = 1;
...
-- Find Total trans. for each customer i.e, SUM(total_amount)
-- Join both the tables using inner join
-- filter the year of transaction for current year dynamically i.e, extract date fn.
-- Group Cust.id , order as per req.
...
WHERE EXTRACT(YEAR FROM t.Transaction_Date) =  EXTRACT(YEAR FROM current_date)  -- transaction_year = current_year (2024)
GROUP BY 1 ;
