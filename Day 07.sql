...
-- Fetch top two highest_grossing products for each category
-- Assign rank for each product on total_spend within each category
-- Group category, product 
-- Filter the year for 2022
...
FROM (
    SELECT 
      ...
      DENSE_RANK() OVER(PARTITION BY category ORDER BY SUM(spend) DESC) rn
    FROM product_spend
